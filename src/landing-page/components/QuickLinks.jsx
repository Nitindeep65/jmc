const quickLinks = [
  ...
  {
    name: 'Web Info Manager', href: 'https://jmc.jk.gov.in/webmanager.html',
        ...
        <div className="grid grid-cols-4 md:grid-cols-8 gap-3">
          {quickLinks.map((link, idx) => (
            <a
              ...
              className="group flex flex-col items-center gap-2 bg-white hover:bg-[#003366] border border-gray-200 hover:border-[#003366] rounded-lg p-4 text-center transition-all duration-200 shadow-sm hover:shadow-md"
            >
              <span className="text-[#003366] group-hover:text-[#FF6600] transition-colors">{link.icon}</span>
              <span className="text-[11px] font-semibold text-gray-700 group-hover:text-white leading-tight transition-colors">{link.name}</span>
            </a>
          ))}
        </div>
      </div>
